...
const PARTY_DATE = new Date("2026-12-26");

function getMiniCalendarDays(year: number, month: number, taskDates: Set<string>) {
...
export default function Dashboard() {
  ...
  // Upcoming tasks (not done, sorted by dueDate)
  const upcoming = tasks
    .filter(t => !t.completed && t.dueDate)
    .sort((a, b) => (a.dueDate! > b.dueDate! ? 1 : -1))
    .slice(0, 5);

  // Mini calendar for Dec 2026
  const calYear = 2026, calMonth = 11; // Dec
  const taskDatesSet = new Set(tasks.map(t => t.dueDate).filter(Boolean) as string[]);
  const { firstDay, daysInMonth } = getMiniCalendarDays(calYear, calMonth, taskDatesSet);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-8)" }}>
      ...
      <div style={{
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          <p style={{ fontSize:"var(--text-sm)", opacity:0.85, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"var(--space-2)" }}>
            🌸 Bienvenida al portal de planning
          </p>
          <h1 style={{ fontFamily:"var(--font-display)", fontSize:"var(--text-2xl)", fontWeight:900, lineHeight:1.1, marginBottom:"var(--space-3)" }}>
            La Quinceañera de Sofía
          </h1>
          <p style={{ fontSize:"var(--text-base)", opacity:0.9 }}>
            26 de Diciembre, 2026 · Vestido rosa claro ya listo ✓
          </p>
          <div style={{ marginTop:"var(--space-6)", display:"flex", gap:"var(--space-6)", flexWrap:"wrap" }}>
            <StatChip label="Tareas completadas" value={`${completedTasks}/${totalTasks}`} />
            <StatChip label="Fondos recibidos" value={`${fundingPct}%`} />
            <StatChip label="Padrinos" value={String(godparents.length)} />
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:"var(--space-4)" }}>
        ...
        <KpiCard
          ...
          sub={`de $${totalPledged.toLocaleString()} prometidos`}
          pct={fundingPct}
          goldBar
        />
        <KpiCard
          ...
          sub={`de $${totalBudget.toLocaleString()} estimado`}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"var(--space-6)" }}>
        ...
        <div className="card-lift" style={{
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4, textAlign:"center" }}>
            {["D","L","M","X","J","V","S"].map(d => (
              <div key={d} style={{ fontSize:"var(--text-xs)", fontWeight:600, color:"var(--color-text-muted)", paddingBottom:4 }}>{d}</div>
            ))}
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                <div key={day} style={{
                  ...
                  color: isParty ? "white" : hasTask ? "var(--color-text)" : "var(--color-text)",
          ...
          <div style={{ marginTop:"var(--space-4)", display:"flex", gap:"var(--space-3)", flexWrap:"wrap" }}>
            <Legend color="hsl(340,55%,62%)" label="Fiesta" />
            <Legend color="hsl(42,80%,62%,0.5)" label="Tarea" />
          </div>
        </div>
        ...
        <div className="card-lift" style={{
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"var(--space-3)" }}>
            {upcoming.length === 0 && (
              <p style={{ color:"var(--color-text-muted)", fontSize:"var(--text-sm)" }}>¡Todas las tareas completadas! 🎊</p>
            )}
            {upcoming.map(t => (
              <div key={t.id} style={{
      ...
      <div style={{
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:"var(--space-3)" }}>
          ...
          {tasks.filter(t => t.completed).length === 0 && (
            <p style={{ color:"var(--color-text-muted)", fontSize:"var(--text-sm)" }}>Aún no hay tareas marcadas como completadas.</p>
          )}
        </div>
      </div>
    </div>
